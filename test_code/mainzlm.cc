...
void API_CALL on_mk_play_event_func(void *user_data, int err_code, const char *err_msg, mk_track tracks[], int track_count) {
   mpp_dec_enc *ctx = (mpp_dec_enc *)user_data;   // 将 user_data 转换成 Zlmediakit Context 结构体指针 
...
cv::VideoCapture cap177(options.rtsp_url4, cv::CAP_GSTREAMER);
  ...
  //   }
  // };
process_video_rtsp(&mpp_dec_enc_);
  // 线程函数3，处理第3个摄像头
  auto thread_func3 = [&]()
  {
    // init value
    ...
    printf("pushing...");
    int ret = mk_media_input_h264(mpp_dec_enc_.media, enc_data177, enc_data_size177, mpp_dec_enc_.dts, mpp_dec_enc_.pts);
    if (ret != 1)
    {
